/* ════════════════════════════════════════════════════════════════════
   Simulador de Flujo de Caja · Skandia · CLIENTE v2 · lógica
   Exclusivo de canal cliente — sin ramas de código público.
   ════════════════════════════════════════════════════════════════════ */
...
// ═══════════════════════════════════════════════════════════════════════
//  SELECT CUSTOM — fiel a sk-input-select + sk-listbox (guía clone)
//  El <select> nativo sigue en el DOM oculto: es la fuente de verdad de
//  .value. Se sincroniza y se dispara 'change' para no tocar la lógica
//  existente (onFuentePeriodicidad, onPeriodicidad, lectura directa de
//  #custom-periodicidad.value).
// ═══════════════════════════════════════════════════════════════════════
let _skOpenState = null; // { wrap, select, panel, activeIndex }

function enhanceSelects(root) {
  root.querySelectorAll('.field__select-wrap').forEach(wrap => {
    if (wrap.dataset.skEnhanced) return;
    wrap.dataset.skEnhanced = 'true';
    initSkSelect(wrap);
  });
}

function syncSkValue(wrap, select) {
  const opt = select.options[select.selectedIndex];
  const valueEl = wrap.querySelector('.field__select-value');
  if (valueEl) valueEl.textContent = opt ? opt.textContent : '';
}

function initSkSelect(wrap) {
  const select = wrap.querySelector('select.periodo-select');
  const inner  = wrap.querySelector('.field__select-inner');
  const label  = inner?.querySelector('.field__floating-label');
  if (!select || !inner) return;

  const valueEl = document.createElement('span');
  valueEl.className = 'field__select-value';
  ...
  wrap.setAttribute('aria-label', select.getAttribute('aria-label') || label?.textContent || 'Selecciona una opción');

  syncSkValue(wrap, select);

  wrap.addEventListener('click', () => {
    if (_skOpenState && _skOpenState.wrap === wrap) closeSkSelect();
    else openSkSelect(wrap, select);
  });
  wrap.addEventListener('keydown', (e) => {
    if (_skOpenState && _skOpenState.wrap === wrap) return; // panel abierto: lo maneja el listener global
    if (['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(e.key)) {
      e.preventDefault();
      openSkSelect(wrap, select);
    }
  });
}

function openSkSelect(wrap, select) {
  if (_skOpenState) closeSkSelect();

  const panel = document.createElement('div');
  panel.className = 'sk-select-panel';
  panel.setAttribute('role', 'listbox');

  [...select.options].forEach((opt, i) => {
    const item = document.createElement('div');
    item.className = 'sk-select-option' + (i === select.selectedIndex ? ' sk-select-option--selected' : '');
  ...
  _skOpenState = { wrap, select, panel, activeIndex: select.selectedIndex };
  positionSkPanel();
  setSkActive(select.selectedIndex);
}

function closeSkSelect() {
  if (!_skOpenState) return;
  const { wrap, panel } = _skOpenState;
  panel.remove();
...
function positionSkPanel() {
  ...
  panel.classList.toggle('sk-select-panel--flipped', flip);
  panel.style.left  = Math.round(r.left) + 'px';
  panel.style.width = Math.round(Math.max(r.width, 140)) + 'px';
  panel.style.top   = Math.round(flip ? r.top - panelH - 8 : r.bottom + 8) + 'px';
}

function setSkActive(idx) {
  if (!_skOpenState) return;
  const items = _skOpenState.panel.querySelectorAll('.sk-select-option');
  idx = Math.max(0, Math.min(items.length - 1, idx));
  items.forEach((it, i) => it.classList.toggle('sk-select-option--active', i === idx));
  items[idx]?.scrollIntoView({ block: 'nearest' });
  _skOpenState.activeIndex = idx;
}

document.addEventListener('scroll', () => positionSkPanel(), true);
window.addEventListener('resize', () => positionSkPanel());

document.addEventListener('mousedown', (e) => {
  if (_skOpenState && !_skOpenState.wrap.contains(e.target) && !_skOpenState.panel.contains(e.target)) {
    closeSkSelect();
  }
});
...
document.addEventListener('keydown', (e) => {
  if (!_skOpenState) return;
  const { select, wrap, activeIndex } = _skOpenState;
  if (e.key === 'ArrowDown') { e.preventDefault(); setSkActive(activeIndex + 1); }
  else if (e.key === 'ArrowUp') { e.preventDefault(); setSkActive(activeIndex - 1); }
  else if (e.key === 'Home') { e.preventDefault(); setSkActive(0); }
  else if (e.key === 'End') { e.preventDefault(); setSkActive(select.options.length - 1); }
...
// ─── Inicialización del portal (avatar, saludo, fuente principal) ────
function initPortal() {
...
// ═══════════════════════════════════════════════════════════════════════
//  PANTALLA 1 — INGRESOS (múltiples fuentes)
// ═══════════════════════════════════════════════════════════════════════
          ...
          <div class="fuente-input-row">
            <div class="field__input-wrap fuente-input-wrap">
              ...
              <div class="field__inner">
                ...
                >
                <label class="field__floating-label">Monto mensual</label>
              </div>
            </div>
            <div class="field__select-wrap">
              <div class="field__select-inner">
                <label class="field__floating-label">Periodicidad</label>
                <select
              ...
              <i data-lucide="chevron-down" class="field__select-icon"></i>
            </div>
...
function onFuenteInput(id, input) {
  ...
  const errEl = input.closest('.fuente-row')?.querySelector('.fuente-input-error');
  if (errEl) { errEl.textContent = ''; errEl.hidden = true; }
  input.closest('.field__input-wrap')?.classList.remove('field__input-wrap--error');
...
function onFuenteBlur(id, input) {
  ...
  if (num === 0) {
    ...
    errEl.hidden = false;
    input.closest('.field__input-wrap')?.classList.add('field__input-wrap--error');
  } else {
    errEl.textContent = '';
    errEl.hidden = true;
    input.closest('.field__input-wrap')?.classList.remove('field__input-wrap--error');
        ...
        <div class="amount-row">
          <div class="field__input-wrap" style="flex:1">
            ...
            <div class="field__inner">
              ...
              >
              <label class="field__floating-label">Monto</label>
            </div>
          </div>
          <div class="field__select-wrap">
            <div class="field__select-inner">
  ...
  const items = detalle.map(d => {
    ...
    return `
      <div class="detalle-item" id="detalle-item-${key}-${d.key}">
        ...
        <div class="field__input-wrap detalle-item__input-wrap">
          <span class="field__prefix">$</span>
          <div class="field__inner">
            <input type="text" inputmode="numeric" class="field__input detalle-item__input"
...
const PERFILES = {
  saludable: {
    ...
    cssColor: 'var(--feedback-success-dark)', tone: 'green', icon: 'check-circle-2'
  },
  ajustado: {
    title: 'Tu flujo está ajustado',
    sub:   'Tus ingresos alcanzan, pero el margen es estrecho.',
    cssColor: 'var(--feedback-warning-dark)', tone: 'yellow', icon: 'alert-triangle'
  },
  riesgo: {
    title: 'Tu flujo está en riesgo',
    sub:   'Este mes tus gastos superan tus ingresos.',
    cssColor: 'var(--feedback-error-dark)', tone: 'red', icon: 'alert-circle'
  }
};
...
function renderDiagnostico() {
  ...
  const futuroPct   = state.futuro;                          // directo desde state
  ...
  // Margen disponible (sin contar el ahorro)
  ...
  // Mini-CTA que lleva a Pantalla 4
  ctasEl.innerHTML = `
    <div class="ctas">
  ...
  if (track) track.innerHTML = items.map((it, i) => `
    <div class="carousel__slide${i === idx ? ' carousel__slide--active' : ''}" data-index="${i}">
      <div class="proyeccion${it.rec ? ' proyeccion--rec' : ''}">
        ...
        <button type="button" class="btn btn--secondary btn--small proyeccion__ver-proy"
...
function abrirModalProyeccion(futuroPesos, meses) {
  ...
  modal.querySelectorAll('.modal__time-btn').forEach(btn => {
    const active = Number(btn.dataset.meses) === meses;
    btn.classList.toggle('modal__time-btn--active', active);
...
function cambiarHorizonteModal(meses, btn) {
  document.querySelectorAll('.modal__time-btn').forEach(b => {
    b.classList.remove('modal__time-btn--active');
...
function enviarPDF() {
  mostrarToast('Tu diagnóstico fue enviado a tu correo registrado en Skandia');
}
...
function _mostrarConfirmacion() {
  const overlay = document.getElementById('modal-confirmacion');
  if (!overlay) return;
  const msg = 'Le hemos comunicado a tu asesor tu necesidad de contacto. Pronto se pondrá en comunicación contigo.';
